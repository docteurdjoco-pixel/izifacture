...
import { getClients, createInvoice, Client, InvoiceItem } from '@/lib/data';
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-surface border border-border rounded-xl shadow-sm p-6 space-y-8">
        ...
        <div className="border-t border-border pt-6 flex gap-4 justify-end">
          <button onClick={() => handleSubmit('Brouillon')} disabled={isSaving} className="px-6 py-2.5 rounded-lg text-sm font-medium text-text bg-gray-100 hover:bg-gray-200 transition-colors flex items-center gap-2">
            <Save size={18} /> {isSaving ? 'Sauvegarde...' : 'Sauvegarder Brouillon'}
          </button>
          <button onClick={() => handleSubmit('Envoyée')} className="px-6 py-2.5 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-colors flex items-center gap-2" disabled={!clientId || isSaving}>
